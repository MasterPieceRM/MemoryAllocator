...
void *os_calloc_checked(size_t nmemb, size_t size) {
    ...
    if (nmemb * size != 0) {
        ...
        for (unsigned int i = 0; i < size; i++)
            FAIL(*((char *)(ptr + i)) != 0,
                 "DBG: os_calloc returned uninitialized memory");
    }
...
void *os_realloc_checked(void *ptr, size_t size) {
    ...
    if (size == 0) {
        FAIL(ptr_realloc != NULL,
             "DBG: os_realloc returned NULL on valid size");
        return ptr_realloc;
    } else if (oldBlock.status == STATUS_FREE) {
        FAIL(ptr_realloc != NULL,
             "DBG: os_realloc should return NULL for free blocks");
        return ptr_realloc;
    }

    if (oldBlock.status == STATUS_ALLOC)
        FAIL(memcmp(ptr_realloc, ptr, MIN(oldBlock.size, size)) != 0,
             "DBG: os_realloc corrupted memory");

    return ptr_realloc;
}

void *mock_preallocate(void) { return os_malloc(MOCK_PREALLOC); }
